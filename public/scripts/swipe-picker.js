...
export function initSwipePicker() {
    ...
    // Swipe jump input: prevent clicks from bubbling to the swipe picker
    $(document).on('click', '.swipe-jump-input', function (e) {
        e.stopPropagation();
    });

    // Swipe jump input: jump to swipe on Enter or blur
    async function handleSwipeJump(input) {
        const mesId = Number($(input).closest('.mes').attr('mesid'));
        ...
        if (!Number.isInteger(targetNum) || targetNum < 1 || targetNum > message.swipes.length) {
            // Reset to current value
            input.value = currentSwipeId + 1;
            return;
        }

        const targetSwipeId = targetNum - 1;
        if (targetSwipeId === currentSwipeId) return;

        const direction = targetSwipeId > currentSwipeId ? SWIPE_DIRECTION.RIGHT : SWIPE_DIRECTION.LEFT;
        await swipe(null, direction, { source: SWIPE_SOURCE.SWIPE_PICKER, forceMesId: mesId, forceSwipeId: targetSwipeId });
    }

    $(document).on('keydown', '.swipe-jump-input', async function (e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            await handleSwipeJump(this);
            this.blur();
        }
    });

    $(document).on('blur', '.swipe-jump-input', async function () {
        await handleSwipeJump(this);
    });

    // Greeting search: toggle panel visibility
    $(document).on('click', '.greeting-search-toggle', function (e) {
        e.preventDefault();
        e.stopPropagation();
        const panel = $(this).siblings('.greeting-search-panel');
        panel.toggle();
        if (panel.is(':visible')) {
            panel.find('.greeting-search-input').focus();
        }
    });

    // Greeting search: search and jump on input
    $(document).on('input', '.greeting-search-input', async function () {
        const query = $(this).val().toLowerCase().trim();
        const matchesDisplay = $(this).siblings('.greeting-search-matches');
        const mesElement = $(this).closest('.mes');
        const mesId = Number(mesElement.attr('mesid'));
        const message = chat[mesId];

        if (!query || !message?.swipes?.length) {
            matchesDisplay.text('');
            return;
        }
        ...
        for (let i = 0; i < message.swipes.length; i++) {
            if (String(message.swipes[i] ?? '').toLowerCase().includes(query)) {
                matchingIndices.push(i);
            }
        }

        if (matchingIndices.length === 0) {
            matchesDisplay.text('0 matches');
            return;
        }

        const currentSwipeId = message.swipe_id ?? 0;
        // Find the first match at or after the current position, or wrap to the first match
        let targetIdx = matchingIndices.find(i => i >= currentSwipeId) ?? matchingIndices[0];
        // If we're already on a match, show which match number we're on
        const matchPosition = matchingIndices.indexOf(currentSwipeId);
        const displayPos = matchPosition >= 0 ? matchPosition + 1 : (matchingIndices.indexOf(targetIdx) + 1);
        matchesDisplay.text(`${displayPos}/${matchingIndices.length}`);

        if (targetIdx !== currentSwipeId) {
            const direction = targetIdx > currentSwipeId ? SWIPE_DIRECTION.RIGHT : SWIPE_DIRECTION.LEFT;
            await swipe(null, direction, { source: SWIPE_SOURCE.SWIPE_PICKER, forceMesId: mesId, forceSwipeId: targetIdx });
        }
    });

    // Greeting search: cycle to next match on Enter
    $(document).on('keydown', '.greeting-search-input', async function (e) {
        if (e.key === 'Enter') {
            e.preventDefault();
            const query = $(this).val().toLowerCase().trim();
            const matchesDisplay = $(this).siblings('.greeting-search-matches');
            ...
            if (!query || !message?.swipes?.length) return;
            ...
            const currentSwipeId = message.swipe_id ?? 0;
            // Jump to next match after current position
            const nextMatch = matchingIndices.find(i => i > currentSwipeId) ?? matchingIndices[0];
            const displayPos = matchingIndices.indexOf(nextMatch) + 1;
            matchesDisplay.text(`${displayPos}/${matchingIndices.length}`);

            if (nextMatch !== currentSwipeId) {
                const direction = nextMatch > currentSwipeId ? SWIPE_DIRECTION.RIGHT : SWIPE_DIRECTION.LEFT;
                await swipe(null, direction, { source: SWIPE_SOURCE.SWIPE_PICKER, forceMesId: mesId, forceSwipeId: nextMatch });
            }
        } else if (e.key === 'Escape') {
            $(this).closest('.greeting-search-panel').hide();
        }
    });
}
